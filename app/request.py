...
similar_movies_url=app.config['SIMILAR_MOVIES_URL']
...
def get_video(id):
    ...
    with urllib.request.urlopen(get_movie_details_url)as url:
        ...
        if movie_details_response['results'] :
            id = movie_details_response.get('id')
            videos = movie_details_response.get('videos')
                    
            if(  movie_details_response['results'][0].get('type') == "Trailer"):
                key = movie_details_response['results'][0].get('key')
            elif(movie_details_response['results'][1].get('type') == "Teaser" or  movie_details_response['results'][1].get('type') == "Trailer" ):
                key=movie_details_response['results'][1].get('key')
            elif(movie_details_response['results'][2].get('type') == "Teaser" or  movie_details_response['results'][2].get('type') == "Trailer" ):
        ...
        video_object = Video(id,key,videos)
